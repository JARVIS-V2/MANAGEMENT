...
class JARVISHUBTelegramHandler:
    ...
    def command(
        ...
        def _command(func):
            try:
                ...
                LOGGER.debug(
                    f"[CMD] Loaded handler {command} for function {func.__name__} in group {group}"
                )
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                ...
                LOGGER.debug(
                    f"[CMD] Loaded handler {command} for function {func.__name__}"
                )
    ...
    def message(
        ...
        def _message(func):
            try:
                ...
                LOGGER.debug(
                    f"[MSG] Loaded filter pattern {pattern} for function {func.__name__} in group {group}"
                )
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                ...
                LOGGER.debug(
                    f"[MSG] Loaded filter pattern {pattern} for function {func.__name__}"
                )
    ...
    def callbackquery(self, pattern: str = None, run_async: bool = True):
        def _callbackquery(func):
            ...
            LOGGER.debug(
                f"[CALLBACK] Loaded callbackquery handler with pattern {pattern} for function {func.__name__}"
            )
            return func
    ...
    def inlinequery(
        ...
        def _inlinequery(func):
            ...
            LOGGER.debug(
                f"[INLINE] Loaded inlinequery handler with pattern {pattern} for function {func.__name__} | PASSES USER DATA: {pass_user_data} | PASSES CHAT DATA: {pass_chat_data} | CHAT TYPES: {chat_types}"
            )
            return func
